...
export default {
    ...
    methods: {
        ...
        optionFinish() {
            this.$set(this.optionValue, 'array', this.optionArray)
            if( this.optionArray == 1) {
                this.$set(this.optionValue, 'title1', this.optionData[0].title)
                let len = this.optionData[0].chips.length
                for(let i=0; i < len; i++) {
                    this.$set(this.optionValue.options[i], 'name1', this.optionData[0].chips[i]); 
                }
            } else if ( this.optionArray == 2) {
                this.$set(this.optionValue, 'title1', this.optionData[0].title)
                this.$set(this.optionValue, 'title2', this.optionData[1].title)
                let len = this.optionData[0].chips.length
                let len2 = this.optionData[1].chips.length
                for(let i=0; i < len; i++) {
                    for(let j=0; j < len2; j++) {
                        this.$set(this.optionValue.options[i].values[j], 'name1', this.optionData[0].chips[i]); 
                        this.$set(this.optionValue.options[i].values[j], 'name2', this.optionData[1].chips[j]); 
                    }
                }
            } else if ( this.optionArray == 3) {
                this.$set(this.optionValue, 'title1', this.optionData[0].title)
                this.$set(this.optionValue, 'title2', this.optionData[1].title)
                this.$set(this.optionValue, 'title3', this.optionData[2].title)
                let len = this.optionData[0].chips.length
                let len2 = this.optionData[1].chips.length
                ...
                for(let i=0; i < len; i++) {
                    for(let j=0; j < len2; j++) {
                        for(let k=0; k < len3; k++) {
                            this.$set(this.optionValue.options[i].values[j].element[k], 'name1', this.optionData[0].chips[i]); 
                            this.$set(this.optionValue.options[i].values[j].element[k], 'name2', this.optionData[1].chips[j]); 
                            this.$set(this.optionValue.options[i].values[j].element[k], 'name3', this.optionData[2].chips[k]); 
                        }
                    }
                }
            }
            if ( this.optionDefault == true ) {
                alert("옵션설정이 완료되었습니다.")
                this.$emit("option-value-default-parent", this.optionValue);
            } else {
                alert("옵션설정이 취소되었습니다.")
